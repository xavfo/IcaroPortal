...
<table width="97%" border="0" cellspacing="0" cellpadding="0" align="center">
	<tr>
		<td>
			<html:form action="/system/container/document">
...
<c:choose>
	<%--<c:when test="${empty requestScope.requestList && listRequestForm.listItems}">--%>		
	<c:when test="${empty requestScope.documentList && documentContainerForm.listItems }">
	...
	<c:forEach var="item" items="${documentList}" varStatus="status">	
		<c:url value="/system/container/document.do" var="urlRead">
		...
		<tr class="item" onMouseOver="swapClass(this, 'item', 'itemhi')" onMouseOut="swapClass(this, 'item', 'itemhi')">
			...
			<td><a href="${urlRead}" class="item">${item.title}</a>&nbsp;(<fmt:formatNumber value="${item.size/1024}" pattern="##.00"/> Kb)</td>
			...
			<td><c:if test="${!empty item.path}"><a href="${pageContext.request.contextPath}${item.path}" target="_blank" alt="${item.path}"><img src="${pageContext.request.contextPath}${item.pathIcon}" border="0" alt="${item.path}"></a>&nbsp;</c:if></td>
		</tr>
	</c:forEach>
	</c:otherwise>
	</c:choose>	
