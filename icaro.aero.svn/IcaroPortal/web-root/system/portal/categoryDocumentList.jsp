...
<c:url value="/system/catalog/category.do" var="urlBack">
	<c:param name="action" value="read" />
	<c:param name="code" value="${category.code}" />
</c:url>
<table border="0" cellpadding="0" cellspacing="0" width="100%">
	<tr>
		<td align="right"><bean:message key="label.backTo" bundle="messages" />:<a href="${urlBack}" class="back"> ${category.name}</a></td>
	</tr>
</table>
...
<html:form action="/system/portal/categoryDocument">
<html:hidden property="action" value="" />
<html:hidden property="categoryCode" value="${category.code}" />
<!-- Options -->
<div>
...
<table class="list" border="0" cellpadding="2" cellspacing="1" width="100%">
	...
	<c:forEach var="item" items="${documentList}" varStatus="status">
		<c:url value="/system/portal/categoryDocument.do" var="urlRead">
			<c:param name="action" value="read" />
