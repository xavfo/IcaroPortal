...
<table border="0" cellpadding="0" cellspacing="0" width="100%">
	<tr>
		<td class="title" width="300">
			...
			<c:set var="systemMenuLabel" ><bean:message key="information.state" bundle="systemMenu" /></c:set>
		</td>
		<td align="right">
...
<html:form action="/system/portal/state/save" focus="name" onsubmit="return preValidate(this)">
...
<table border="0" cellpadding="0" cellspacing="0" width="100%">
	<tr>
		<td class="subtitle">
			<c:choose>
				<c:when test="${!empty stateForm.code && stateForm.code != 0}">
					<bean:message key="message.edit" bundle="systemHelp" arg0="${stateForm.name}" />
				</c:when>
				<c:otherwise>
...
<html:javascript formName="stateForm" dynamicJavascript="true" staticJavascript="false" />
