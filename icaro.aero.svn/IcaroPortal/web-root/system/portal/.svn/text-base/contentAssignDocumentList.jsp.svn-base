		...
		<td class="title" nowrap="nowrap">		
			...
			<c:choose>
				<c:when test="${documentContainerForm.docTypeCode eq initParameters.DOCUMENT_TYPE_DOCUMENTS}">
					<bean:message key="information.related.documents" bundle="systemMenu" />
					<c:set var="systemMenuLabel" ><bean:message key="document.document" bundle="systemMenu" /></c:set>
					<c:set var="systemMenuLabe2" ><bean:message key="information.selection.documents" bundle="systemMenu" /></c:set>
				</c:when>
				<c:otherwise>
					...
					<c:set var="systemMenuLabe2" ><bean:message key="information.selection.support" bundle="systemMenu" /></c:set>
				</c:otherwise>
			</c:choose>
		</td>
		<td align="right">
				...
				<tr>
					<td>
						...
						<table width="97%" border="0" cellspacing="0" cellpadding="0" align="center">
							<tr>
								<td>
									<html:form action="/system/portal/content/document">
									<html:hidden property="action" value="listAssigned" />
						...
						<table border="0" cellpadding="0" cellspacing="0" width="100%">
							<tr>
								<td align="center">
									<html:button property="button" style="font-size: 9px; height: 18px;" styleClass="button" onclick="sendList(this);" onmouseover="swapClass(this, 'button', 'buttonhi')" onmouseout="swapClass(this, 'button', 'buttonhi')">
										${systemMenuLabe2}
									</html:button>
								</td>
							</tr>
						...
						<table align="center" class="list" border="0" cellpadding="2" cellspacing="1" width="95%">
							...
							<!--Fin Header-->
								<c:forEach var="item" items="${documentList}" varStatus="status">	
									<c:url value="/system/portal/content/document.do" var="urlRead">
