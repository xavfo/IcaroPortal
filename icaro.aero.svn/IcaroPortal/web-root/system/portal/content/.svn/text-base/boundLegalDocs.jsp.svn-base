...
<script>
	function openAll(name){
		location.href='<%=request.getContextPath()%>/system/portal/content/legalDoc.do?action=listUnbound&itemCode=${currentContent.code}'
	}
	function resetTitle(){
...
<table border="0" cellpadding="0" cellspacing="0" width="100%">
	<tr>
		<td class="title" width="65%">
			${currentContent.name} -
			<bean:message key="prompt.legal.documents" />
		</td>
		<td align="right">
				...
				<tr>
					<td>
						...
						<table align="center" class="list" border="0" cellpadding="2" cellspacing="1" width="95%">
							...
							<!--Fin Header-->
							<c:forEach var="item" items="${boundLegalDocList}" varStatus="status">	
								<%--
								<c:url var="urlRead" value="/system/container/legalDoc.do">	
