...
<html:form action="/system/admin/sysModule" target="iform">
	...
	<tr>
		<td>
			...
			<!--
				...
				tree.add(0,-1,'<bean:message key="label.system" bundle="messages" />');
					
				<c:forEach var="item" items="${sysModuleList}" varStatus="status">
			       	<c:url value="/system/admin/sysModule.do" var="urlRead">
					...
					<c:choose>
						...
						<c:otherwise>
							tree.add(${item.code}, <c:out value="${item.parent.code}" default="0" />, '${item.name}', '${urlRead}', '', 'iform');
						</c:otherwise>
					</c:choose>
				</c:forEach>
