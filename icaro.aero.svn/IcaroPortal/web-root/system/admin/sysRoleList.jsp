...
<table align="center" class="list" border="0" cellpadding="2" cellspacing="1" width="95%">
	<tr>
		...
		<th width="200"><bean:message key="label.name" bundle="messages" /></th>
		<th><bean:message key="label.description" bundle="messages" /></th>
		<th width="100"><bean:message key="label.profile" bundle="messages" /></th>
	</tr>
	<c:forEach var="item" items="${sysRoleList}" varStatus="status">
		<c:url value="/system/admin/sysRole.do" var="urlRead">
			<c:param name="action" value="read" />
			<c:param name="code" value="${item.code}" />
		</c:url>
		<c:url value="/system/admin/sysRole.do" var="urlSection">
			<c:param name="action" value="listSections" />
			<c:param name="code" value="${item.code}" />
		</c:url>
		<c:url value="/system/admin/sysProfile.do" var="urlProfile">
			<c:param name="action" value="list" />
			<c:param name="roleCode" value="${item.code}" />
		</c:url>
		<tr class="item" onMouseOver="swapClass(this, 'item', 'itemhi')" onMouseOut="swapClass(this, 'item', 'itemhi')">
			...
			<td><a href="${urlProfile}" class="item"><bean:message key="label.profile" bundle="messages" /></a></td>
		</tr>
	</c:forEach>
</table>
</html:form>