...
<table border="0" cellpadding="0" cellspacing="0" width="100%">
	<tr>
		<td class="title" width="300"><bean:message key="cart.paymentType" bundle="systemMenu" /></td>
		<td align="right">
...
<table width="97%" border="0" cellspacing="0" cellpadding="0" align="center">
	<tr>
		<td>
			<html:form action="/system/cart/paymentType">
						...
						<table border="0" cellpadding="0" cellspacing="2" width="100%">
							<tr>
								<td width="250">
									...
									<html:text property="name" styleClass="input" style="width: 240px;" maxlength="100" />
								</td>
								<td>									
						...
						<tr>
							...
							<td>
								<html:select property="orderField" size="1" styleClass="input" style="width: 110px;" onchange="submitOrder(this)">
									...
									<html:option value="enabled"><bean:message key="prompt.enabled"  /></html:option>
									<html:option value="restricted"><bean:message key="prompt.restricted"  /></html:option>
								</html:select>				
							</td>
							<td><img src="${pageContext.request.contextPath}/images/shim.gif" width="5" height="15" ></td>
							<td>
	...
	<tr>
		...
		<td align="left">
			<tiles:insert definition="system.navBar">
				<c:if test="${initParameters.SYS_ROLE_SYSTEM eq sysUser.role.code }">
...
<table class="list" border="0" cellpadding="2" cellspacing="1" width="100%">
	<tr>
		...
		<th width="80"><bean:message key="prompt.enabled"/></th>
		<th width="80"><bean:message key="prompt.restricted"  /></th>
		<th><bean:message key="prompt.logo"  /></th>
	</tr>
	<c:forEach var="item" items="${paymentTypeList}" varStatus="status">		
		<c:url value="/system/cart/paymentType.do" var="urlRead">
		...
		<tr class="item" onMouseOver="swapClass(this, 'item', 'itemhi')" onMouseOut="swapClass(this, 'item', 'itemhi')">
			...
			<td>
				<c:choose>
					<c:when test="${item.restricted == true}">
						<bean:message key="label.yes" bundle="messages" />
					</c:when>
					<c:otherwise>
						<bean:message key="label.no" bundle="messages" />
			...
			<td>
				<c:if test="${!empty item.logo}"><img src="${pageContext.request.contextPath}${item.logo}" border="0" alt="${item.name}"></c:if>
			</td>
		</tr>
	</c:forEach>
	<c:if test="${empty requestScope.paymentTypeList && paymentTypeForm.listItems }">
		<tr>
			<td height="20" align="center" colspan="6" class="message"><bean:message key="message.empty.records" /></td>
		</tr>	
	</c:if>  
