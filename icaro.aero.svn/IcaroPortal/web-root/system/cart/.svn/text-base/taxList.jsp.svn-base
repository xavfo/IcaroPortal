...
<table border="0" cellpadding="0" cellspacing="0" width="100%">
  <tr>
    <td class="title" width="300"><bean:message key="cart.tax" bundle="systemMenu" /></td>
    <td align="right"><bean:message key="cart" bundle="systemMenu" /> &lt; <bean:message key="cart.tax"
      bundle="systemMenu" /></td>
  </tr>
</table>
...
<table border="0" cellpadding="0" cellspacing="0" width="100%">
  <tr>
    ...
    <span class="error"><html:errors /></span></td>
  </tr>
</table>
<img src="${pageContext.request.contextPath}/images/shim.gif" width="1" height="25" border="0">
<br>
...
<%--  table width="97%" border="0" cellspacing="0" cellpadding="0" align="center">
	<tr>
		<td>
			<html:form action="/system/cart/tax">
					...
					<td width="100%" bgcolor="#EBEEF3">
						...
						<table border="0" cellpadding="0" cellspacing="2" width="100%">
							<tr>
								...
								<td width="170">
									<bean:message key="label.paymentStatus" bundle="messages" /><br>
									<html:select property="paymentStatusCode" size="1" styleClass="input" style="width: 150px;">
										<html:option value="0"><bean:message key="option.all" bundle="messages"/></html:option>
										<html:options collection="paymentStatusList" property="code" labelProperty="name"/>
									</html:select>
								</td>
								<td width="120">
						...
						<tr>
							...
							<td>
								<html:select property="orderField" size="1" styleClass="input" style="width: 145px;" onchange="submitOrder(this)">
									...
									<html:option value="payment.status.name"><bean:message key="label.paymentStatus" bundle="messages" /></html:option>
...
<table border="0" cellpadding="0" cellspacing="0" width="100%">
  <tr>
    <td><tiles:insert definition="tabs.system.taxType">
      <tiles:put name="code" value="${taxForm.code}" />
      <tiles:put name="tab" value="${tab}" />
    </tiles:insert></td>
  </tr>
  <tr>
    <td><html:form action="/system/cart/tax">
      ...
      <table class="tabForm" border="0" cellpadding="0" cellspacing="4" width="100%">
        ...
        <tr>
          ...
          <table border="0" cellpadding="0" cellspacing="0" width="100%">
            <tr>
              <td></td>
              <td align="left"><tiles:insert definition="system.navBar">
          ...
          <table align="center" class="list" border="0" cellpadding="2" cellspacing="1" width="95%">
            <tr>
              ...
              <th width="30"><input type="checkbox" name="checkAll"
                onClick="setCheckBoxValue(this.form, 'codes', this.checked)"></th>
              <%-- th width="10%"><bean:message key="label.code" bundle="messages" /></th --%>
              <th width="30%"><bean:message key="label.tax" bundle="messages" /></th>
              <th width="20%"><bean:message key="label.taxRate" bundle="messages" /></th>
              <th width="20%"><bean:message key="label.taxSince" bundle="messages" /></th>
              <th width="20%"><bean:message key="label.taxUntil" bundle="messages" /></th>
            </tr>
            <c:forEach var="item" items="${taxRateList}" varStatus="status">
              <c:url value="/system/cart/tax.do" var="urlRead">
                <c:param name="action" value="read" />
                <c:param name="code" value="${item.code}" />
                <c:param name="tab" value="1" />
              </c:url>
              <tr class="item" onMouseOver="swapClass(this, 'item', 'itemhi')"
                onMouseOut="swapClass(this, 'item', 'itemhi')">
                <td>${status.count}</td>
                <td align="center"><html:checkbox property="codes" value="${item.code}" /></td>
                <%--td><fmt:formatNumber value="${item.tax.code}" pattern="#00000000" /></td --%>
                <td><a href="${urlRead}" class="item">${item.tax.name}</a></td>
                <td><fmt:formatNumber value="${item.rate}" pattern="#.0%" /></td>
                <td><fmt:formatDate value="${item.since.time}" pattern="yyyy-MM-dd"/></td>
                <td><fmt:formatDate value="${item.until.time}" pattern="yyyy-MM-dd"/></td>
              </tr>
            </c:forEach>
          </table>
          <br/><br/>
  ...
  <tr>
    <td background="${pageContext.request.contextPath}/images/tables/file_bottom.gif"><img
      src="${pageContext.request.contextPath}/images/shim.gif" width="1" height="7" border="0" alt=""></td>
  </tr>
  <tr>
