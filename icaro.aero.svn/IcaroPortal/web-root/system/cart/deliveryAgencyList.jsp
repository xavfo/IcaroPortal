...
<table border="0" cellpadding="0" cellspacing="0" width="100%">
	<tr>
		<td class="title" width="300"><bean:message key="cart.shipping.deliveryAgency" bundle="systemMenu" /></td>
		<td align="right">
...
<table width="97%" border="0" cellspacing="0" cellpadding="0" align="center">
	<tr>
		<td>
			<html:form action="/system/portal/deliveryAgency">
						...
						<table border="0" cellpadding="0" cellspacing="2" width="100%">
							<tr>
								...
								<td width="130">
									...
									<html:select property="stateCode" size="1" styleClass="input" style="width: 120px;" >
										<html:option value="0"><bean:message key="option.all" bundle="messages" /></html:option>
										<html:options collection="stateList" property="code" labelProperty="name"/>
									</html:select>
								</td>		
								<td width="130">
									<bean:message key="label.route" bundle="messages" /><br>
									<html:select property="routeCode" size="1" styleClass="input" style="width: 120px;"onchange="setState()">
										<html:option value="0"><bean:message key="option.all" bundle="messages" /></html:option>
						...
						<tr>
							...
							<td>
								<html:select property="orderField" size="1" styleClass="input" style="width: 110px;" onchange="submitOrder(this)">
									...
									<html:option value="externalCode"><bean:message key="label.code" bundle="messages" /></html:option>
									<html:option value="address.state.country.name"><bean:message key="label.country" bundle="messages" /></html:option>
									<html:option value="address.state.name"><bean:message key="label.state" bundle="messages" /></html:option>
									<html:option value="route.name"><bean:message key="label.route" bundle="messages" /></html:option>
								</html:select>				
							</td>
							<td><img src="${pageContext.request.contextPath}/images/shim.gif" width="5" height="15" ></td>
							<td>
...
<table class="list" border="0" cellpadding="2" cellspacing="1" width="100%">
	<tr>
		...
		<th width="150"><bean:message key="label.state" bundle="messages" /></th>
		<th width="100"><bean:message key="label.country" bundle="messages" /></th>
		<th width="100"><bean:message key="label.route" bundle="messages" /></th>
		<th width="100"><bean:message key="label.enabled" bundle="messages" /></th>
	</tr>
	<c:forEach var="item" items="${agencyList}" varStatus="status">		
		<c:url value="/system/portal/deliveryAgency.do" var="urlRead">
		...
		<tr class="item" onMouseOver="swapClass(this, 'item', 'itemhi')" onMouseOut="swapClass(this, 'item', 'itemhi')">
			...
			<td>${item.address.state.name}</td>
			<td>${item.address.state.country.name}</td>
	...
	<c:if test="${empty requestScope.agencyList && deliveryAgencyForm.listItems }">
		<tr>
