...
<head>
	...
	function Close(){
		if (document.colorform.hexval.value == "") {
			alert("No existe un color asignado");
			return
		}
		opener.document.forms[<%=request.getParameter("index")%>].elements['<%=request.getParameter("control")%>'].value = document.colorform.hexval.value;
		close();
	}
	//  End -->
	</script>
...
<a><img usemap="#colmap" src="<%=request.getContextPath()%>/images/system/colortable.gif" border=0 width=289 height=67></a><br>
<br>
Color Seleccionado: <input type=text name=hexval size=10 readonly> <input type="button" value="Agregar" onClick="Close();">
</form>
</center>
</body>
</html>
