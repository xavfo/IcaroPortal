			...
			<td align="center">		
				...
				<tr>
					<td>
						...
						<tr>
							<td>		
							<html:form action="/system/portal/route/frequency/flight/save" focus="number" onsubmit="return preValidate(this)">
							<html:hidden property="action" value="save" />
							<html:hidden property="code" />
							<html:hidden property="frequencyCode" value="${frequencyForm.code}" />
							<html:hidden property="departureDate" />
							<html:hidden property="arrivalDate" />
									...
									<td class="subtitle">
										<c:choose>
											<c:when test="${!empty frequencyForm.code && frequencyForm.code != 0}">
												<bean:message key="message.edit" bundle="systemHelp" arg0="${flightForm.number}" />
											</c:when>
											<c:otherwise>
							...
							<table class="form" align="right" border="0" cellpadding="4" cellspacing="0" width="95%">
								...
								<tr>
									<td class="required">* <bean:message key="prompt.number" /> :</td>
									<td><html:text property="number" styleClass="input" style="width: 100px;" maxlength="20" /></td>
								</tr>
								
								<tr>
					              <td class="required">* <bean:message key="prompt.departureTime" /> :</td> 
					              <td>
									<html:select property="departureHour" size="1" styleClass="input" style="width: 60px;">
										<html:options collection="hourList" property="value" labelProperty="label"/>
									</html:select>:
									<html:select property="departureMinute" size="1" styleClass="input" style="width: 60px;">
										<html:options collection="minuteList" property="value" labelProperty="label"/>
									</html:select>

									</td>
					            </tr>
								<tr>
					              <td class="required">* <bean:message key="prompt.arrivalTime" /> :</td> 
					              <td>
									<html:select property="arrivalHour" size="1" styleClass="input" style="width: 60px;">
										<html:options collection="hourList" property="value" labelProperty="label"/>
									</html:select>:
									<html:select property="arrivalMinute" size="1" styleClass="input" style="width: 60px;">
										<html:options collection="minuteList" property="value" labelProperty="label"/>
									</html:select>
									</td>
					            </tr>
							...
							<html:javascript formName="flightForm" dynamicJavascript="true" staticJavascript="false" />
...
<script language="JavaScript1.2">
	...
	function preValidate (frm) { 
	  ...
	  <c:choose>
	  	<c:when test="${!empty flightForm.code && flightForm.code != 0}">
