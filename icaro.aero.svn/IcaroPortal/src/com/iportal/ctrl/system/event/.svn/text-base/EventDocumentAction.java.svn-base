 ...
 *
 */
public class EventDocumentAction extends BaseDispatchAction {
    
	private static Log logger = LogFactory.getLog(EventDocumentAction.class);
    ...
    throws Exception {
    	...
    	results = facade.getAssignedDocuments(code, listForm, Event.class, request);
	...
	/**
	 * Save a new document in the database and assign it to an Event    
	 * @param mapping
	 * @param form
	 * @param request
	 * @param response
    ...
    throws Exception {
    	...
    	try {
			...
			if (eventCode != null ){
				event = (Event)sess.load(Event.class, eventCode);
			...
			try {
			    ...
			    if (codes != null){
			    	for (int i = 0; i < codes.length; i++){
			    		eventDocumentCode = codes[i];
			    		DocumentContainer documentContainer = (DocumentContainer)sess.load(DocumentContainer.class, eventDocumentCode);
